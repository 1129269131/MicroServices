...
public class NacosNamingService implements NamingService {
    ...
    public void registerInstance(String serviceName, String groupName, Instance instance) throws NacosException {
        // day04：生成的String格式为：groupId@@微服务名称
        String groupedServiceName = NamingUtils.getGroupedName(serviceName, groupName);
        // day04：若当前实例为临时实例，则向Server发送心跳
        if (instance.isEphemeral()) {
            // day04：构建一个心跳信息实例
            BeatInfo beatInfo = beatReactor.buildBeatInfo(groupedServiceName, instance);
            // day04：向server发送心跳（定时任务）
    ...
    public List<Instance> selectInstances(String serviceName, String groupName, boolean healthy, boolean subscribe)
            throws NacosException {
        // day08：subscribe的值为true，表示本次查询是 订阅
    ...
    public List<Instance> selectInstances(String serviceName, String groupName, List<String> clusters, boolean healthy,
        ...
        // day08：从serviceInfo的所有instance实例中过滤出所有可用的
    ...
    private List<Instance> selectInstances(ServiceInfo serviceInfo, boolean healthy) {
        ...
        // day08：迭代服务的所有instance实例
        Iterator<Instance> iterator = list.iterator();
        while (iterator.hasNext()) {
            Instance instance = iterator.next();
            // day08：若当前instance不是健康的，或不可用，或其权重小于等于0，则从列表中将其删除
            if (healthy != instance.isHealthy() || !instance.isEnabled() || instance.getWeight() <= 0) {
                iterator.remove();
            }
        }
        // day08：返回的这个列表中包含的instance都是可用的
        return list;
    }
